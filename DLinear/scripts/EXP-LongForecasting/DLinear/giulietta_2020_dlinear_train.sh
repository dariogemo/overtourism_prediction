...
DATA_DIR="$PROJECT_DIR/../main_dataset/count_data_train"
...
data_file=$DATA_DIR/data_casa_di_giulietta_train.csv
...
if [ -f "$data_file" ]; then
  ...
  python -u $PROJECT_DIR/DLinear/run_longExp.py \
    ...
    --d_layers 1 \
    --des 'Exp' \
    --itr 1 > "$PROJECT_DIR/DLinear/logs/LongForecasting/${model_id}_2020_train.log"
fi
