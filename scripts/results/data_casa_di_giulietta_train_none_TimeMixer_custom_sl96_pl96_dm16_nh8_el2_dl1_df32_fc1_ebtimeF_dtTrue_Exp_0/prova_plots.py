...
final_df = pd.DataFrame(
    very_pred[:, 6], index=df_2019.index[: len(very_pred)], columns=["Predicted"]
)
...
print("MAE for timemixer", round(mae, 2))

plt.figure()
plt.title("TimeMixer predictions for Giulietta first 5 days", fontsize=40)
plt.plot(final_df.iloc[:500]["count"], label="True")
plt.plot(final_df.iloc[:500]["Predicted"], label="Predicted")
plt.legend()
# plt.savefig("../../img/predictions/giulietta_timemixer_first500.png")

plt.figure()
plt.title("TimeMixer predictions for Giulietta 2019", fontsize=40)
plt.plot(final_df["count"], label="True")
plt.plot(final_df["Predicted"], label="Predicted")
plt.legend()
# plt.savefig("../../img/predictions/giulietta_timemixer_full.png", dpi=1000)
