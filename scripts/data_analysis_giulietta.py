...
if run_figs == "y" or run_figs == "yes":
    ...
    sns.lineplot(y=df["rain"], x=df.index, linewidth=0.3, color="#ca3ae8", ax=ax[1][0])
    sns.lineplot(
...
week_df: pd.DataFrame = df[week_mask]

if run_figs == "y" and run_figs == "yes":
    fig, ax = plt.subplots(2, 2, figsize=(20, 20))
    plt.suptitle("Giulietta's house variables, week of 2018-05-28", fontsize=40)
    sns.lineplot(
        y=week_df["count"], x=week_df.index, linewidth=1.5, color="#1d4863", ax=ax[0][0]
    )
    sns.lineplot(
        y=week_df["temp"], x=week_df.index, linewidth=3, color="#e8ad3a", ax=ax[0][1]
    )
    ax[0][1].set_ylim(0, 4.3)
    sns.lineplot(
        y=week_df["rain"], x=week_df.index, linewidth=3, color="#ca3ae8", ax=ax[1][0]
    )
    sns.lineplot(
        y=week_df["popularity"],
        x=week_df.index,
        linewidth=3,
        color="#5cad52",
        ax=ax[1][1],
